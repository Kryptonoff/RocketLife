...
try
{
	...
	private _query = format ["getGarageVehicle:%1",_id] call RRPHC_DB_selectSingle;
	...
	if !((_sp_name isEqualTo _spname) OR {_spname isEqualTo ""} OR {_spname isEqualTo "none"}) then
	{
		if (isClass (getMissionConfig "LifeCfgGarages" >> worldName >> _spname) && {!(getMarkerPos _spname isEqualTo [0,0,0])}) then
		{
			...
			format ["setGarage:none:%1",_id] call RRPHC_DB_fireAndForget;
		};
	};

	[_player,"atm","take",_price] remoteExecCall ["RRPServer_system_moneyChange",2];
	[_player,"serverbank","add",_price] remoteExecCall ["RRPServer_system_moneyChange",2];
	format ["setVehicleActive:%1",_id] call RRPHC_DB_fireAndForget;

	["retrieve",true,_id,_query] remoteExecCall ["RRPClient_garage_responseServer",_player];

}catch{
	["retrieve",false,_exception] remoteExecCall ["RRPClient_garage_responseServer",_player];
};
true
