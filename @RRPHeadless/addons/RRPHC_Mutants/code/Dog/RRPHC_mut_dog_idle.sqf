...
[{
	...
	[{
		...
		if ((isNull _myNearestEnemy) or !(alive _myNearestEnemy) or ((_myNearestEnemy distance _mutant) > 30)) then {
			group _mutant setSpeedMode "LIMITED";
			...
			if (animationState _mutant in ["psina_sit_to_lie", "psina_lie_idle"]) then {
				...
				[{
					if (alive _mutant) then {[_mutant, "psina_idle_1"] remoteExec ["switchMove",-2]}
			...
			[_mutant] call RRPHC_mut_findNewPos;
			...
			if (_rndAction == "Sleep") then {
				...
				[{
					if (alive _mutant) then {[_mutant, "psina_sit_to_lie"] remoteExec ["switchMove",-2]}
				}, _mutant, 1] call CBA_fnc_waitAndExecute;

				[{
					if (alive _mutant) then {[_mutant, "psina_lie_idle"] remoteExec ["switchMove",-2]}
				...
				if (_rndAction == "Sit") then {
					if (alive _mutant) then {[_mutant, "psina_idle_to_sit"] remoteExec ["switchMove",-2]};
					[{
						if (alive _mutant) then {[_mutant, "psina_sit_idle"] remoteExec ["switchMove",-2]}
					}, _mutant, 1] call CBA_fnc_waitAndExecute;
