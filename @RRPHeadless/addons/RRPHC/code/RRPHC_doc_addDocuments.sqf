...
try {
	...
	format ["addDocuments:%1:%2:%3:%4:%5:%6:%7",_docOwnerUid,_docClass,_docName,_docDescription,_docTime,_docGiveUid,_docGiveName] call RRPHC_DB_insertSingle;
	_return = format["getDocuments:%1",_docOwnerUid] call RRPHC_DB_selectFull;
	[_return] remoteExecCall ["Client_documents_dataReceived",_docOwner];

	true
} catch {
	diag_log format ["RRPHC_doc_addDocuments ERROR: %1",_exception];
};
