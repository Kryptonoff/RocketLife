...
diag_log "---------------------------------------------------------------------------------------------------- #0101" call RRPHC_util_debugConsole;
diag_log "---------------------------------- Starting Initializing to extDB3 --------------------------------- #0101" call RRPHC_util_debugConsole;
diag_log "------------------------------------------ Version 1032 --------------------------------------------- #0101" call RRPHC_util_debugConsole;
diag_log "---------------------------------------------------------------------------------------------------- #0101" call RRPHC_util_debugConsole;
...
try
{
	...
	format ["Installed extDB3 version: %1 #1001", _result] call RRPHC_util_debugConsole;
	...
	"Connected to database! #0101" call RRPHC_util_debugConsole;
	...
	"Database protocol initialized! #0101" call RRPHC_util_debugConsole;
	if (_is_dev) then 
	{
		"extDB3" callExtension "9:LOCK:aerc3e";
		"extDB3 can be unlocked! Remove password if it not dev server immediatly! #1001" call RRPHC_util_debugConsole;
	}else {
...
catch
{
	...
	{
		"MySQL connection error! #1001" call RRPHC_util_debugConsole;
		"Make sure [Database] in the extdb3-conf.ini is set to [altislife] unless you have a different database setup!!!!! #1001" call RRPHC_util_debugConsole;
		"Please have a look at @extDB3/logs/ to find out what went wrong. #1001" call RRPHC_util_debugConsole;
		format ["MySQL Error: %1 #1001", _exception]  call RRPHC_util_debugConsole;
		"Server will locked now :( #1001" call RRPHC_util_debugConsole;
	}
	else
	{
		format ["extDB3: %1", _exception] call RRPServer_DB_log;
		"Check your server rpt for errors, your mission might be stuck a loop restarting #1001" call RRPHC_util_debugConsole;
	};
};
diag_log "---------------------------------------------------------------------------------------------------- #0101" call RRPHC_util_debugConsole;
diag_log format["               End of Init extDB3 :: Total Execution Time %1 seconds  #0101",(diag_tickTime) - _timeStamp] call RRPHC_util_debugConsole;
diag_log "---------------------------------------------------------------------------------------------------- #0101" call RRPHC_util_debugConsole;
