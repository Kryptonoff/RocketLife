...
if !(_db isEqualTo -1) then
{
	...
	_query = ["updateVehicles",_data] call RRPHC_DB_createMessage;
	_query call RRPHC_DB_fireAndForget;
	format ["Vehicle: %1 ID: %2 Saved #0101",typeOf _vehicle,_db] call RRPHC_util_debugConsole;
}else{
	format ["Vehicle: %1 ID: %2 Not Saved: DB must be != -1 #0101",typeOf _vehicle,_db] call RRPHC_util_debugConsole;
};
true
