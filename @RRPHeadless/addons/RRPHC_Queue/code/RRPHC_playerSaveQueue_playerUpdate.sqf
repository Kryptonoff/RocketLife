...
if (isNull _unit) exitWith {
	format ["Player undefined ??? %1 #1001",str _unit] call RRPHC_util_debugConsole;
};
private _name = name _unit;
if (_unit getVariable ["inJail", false]) exitWith {format ["Player In Jail (dnt save) - N:%1 U:%2 #0111",name _unit,getPlayerUID _unit] call RRPHC_util_debugConsole;false};
...
private _message = ["updatePlayer", _data] call RRPHC_DB_createMessage;
_message call RRPHC_DB_fireAndForget;

format ["Player %1(%3) saved - time %2s #0111",name _unit,diag_tickTime - _time,_uid] call RRPHC_util_debugConsole;
true
