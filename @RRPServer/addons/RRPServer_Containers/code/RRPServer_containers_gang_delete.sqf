...
try
{
	switch (_action) do
	{
		case "check": {
			...
			["response",_item,_num] remoteExecCall ["RRPClient_containers_gang_delete",remoteExecutedOwner];
		};
		case "save": {
			format ["ContainersDelete: Saved %1 #0101",_box] call RRPServer_util_debugConsole;
			_box setVariable ["inventory",[_item,_num],true];
			[netId _box] call RRPServer_containers_gang_UpdateDB;
			["success"] remoteExecCall ["RRPClient_containers_gang_delete",remoteExecutedOwner];
		};
	};
}catch{
	format ["ContainersDelete: %1 #1001",_exception] call RRPServer_util_debugConsole;
	["error",objNull,_exception] remoteExecCall ["RRPClient_containers_gang_delete",remoteExecutedOwner];
};
true
