...
if !(isNull _obj) then 
{
	...
	{
		_item = _x;
		if ((_item select 0) isEqualType []) then {_item = _item#0};
		_itemDetails = [_item#0] call RRPClient_util_itemDetails;
		if (_itemDetails isEqualTo []) then 
		{
			format ["GangBox Error: Предмета %1 в ящике %2 не существует в моде, предмет удален #1001",_item#0,_vardb] call RRPServer_util_debugConsole;
			_inv deleteAt _forEachIndex;
			_saveRequired = true;
		}else{
			_weight = _weight + ((_itemDetails select 6) * (_item select 1));
		};
	}forEach _inv;
