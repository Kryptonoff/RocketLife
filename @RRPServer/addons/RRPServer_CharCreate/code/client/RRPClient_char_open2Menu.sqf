...
params ["_name","_surName"];
createDialog "RRPDialogCharCreate";
_display = uiNamespace getVariable ["RRPDialogCharCreate",displayNull];
_display displayAddEventHandler ["KeyDown", "if ((_this select 1) == 1) then {true};"];
...
_surNameEdit ctrlSetText _surName;

{_x ctrlEnable false}forEach [_nameEdit,_surNameEdit];
...
_accessoriesList lbSetData [_index,""];
{
	...
	_accessoriesList lbSetData [_index,_name];
