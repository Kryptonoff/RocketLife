...
// send frequencies
_freq = if (isNil "PhoneTaskForceRadioFreq") then {["No_SW_Radio"]}else{[PhoneTaskForceRadioFreq]};
...
if (((call TFAR_fnc_haveSWRadio) or (TFAR_currentUnit != player)) and {[TFAR_currentUnit, _isolated_and_inside, _can_speak, _depth] call TFAR_fnc_canUseSWRadio}) then {
	if (isNil "PhoneTaskForceRadioFreq") then {
		_freq = [];
	};
...
if (_alive) then {
	TFAR_player_name = player getVariable ["nameTS",""]; //тут как у вас переименуйте
};
