...
RRPClientForbiddenStringedDiks = ["327693","327692","327699","327703","327702","327698","327697","327701","327700","327696","327788"];
RRPClientKeybindsUp = createLocation ["ExileLife_Dummy", [-5000,-5000,0], 0, 0];
RRPClientKeybindsDown = createLocation ["ExileLife_Dummy", [-5000,-5000,0], 0, 0];
...
]call RRPClient_system_keybinding_add;
	...
	{},
	{
		if !(ExileClientIsHandcuffed || ExileIsPlayingRussianRoulette) then
		{
			if !(ExileClientIsInConstructionMode) then
			{
				...
				{
					[true] call RRPClient_object_player_action_holsterweapon;
				}
				else
				{
					if (player hasWeapon ExileLifeCurrentWeapon) then
					{
						[false] call RRPClient_object_player_action_holsterweapon;
					};
				};
			};
		};
...
[
	...
	},
	{
		...
		if (!(ExileClientIsHandcuffed) || !(((locked ExileClientInteractionObject) isEqualTo 2))) then
		{
			...
			{
				call RRPClient_resources_pickfruitRequest;
			}
			else
			{
				...
				{
					call RRPClient_gui_hud_VehicleActionFind;
				};
			};
		};
		true
