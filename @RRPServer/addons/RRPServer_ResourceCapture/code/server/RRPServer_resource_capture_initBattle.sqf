...
["add",_pointName,"сервер"] remoteExecCall ["AvalonClient_martiallaw_updateWarPoints",-2];
for "_i" from 1400 to 300 step -100 do {
	...
	["circle",_captureArray select 1,"10"] remoteExecCall ["AvalonClient_martiallaw_updateWarPoints",civilian];
	uiSleep (10 * 60);
};
...
BattleIndex = -1;
publicVariable "BattleIndex";
["remove",_pointName,"сервер"] remoteExecCall ["AvalonClient_martiallaw_updateWarPoints",-2];
...
format ["endBattle:%1:%2:%3","Никто",[],_captureArray select 0] call RRPServer_DB_fireAndForget;
