...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<infiSTAR.de> VERSION: infiSTAR.de 22-05-2018 16-16-48 (v259)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='a3atlaslife@gmail.com';
INFISTARVERSION='22-05-2018 16-16-48 - v259';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    a3atlaslife@gmail.com - 22-05-2018 16-16-48 - v259 - %2 - %3',time,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [a3atlaslife@gmail.com - 22-05-2018 16-16-48 - v259 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    a3atlaslife@gmail.com - 22-05-2018 16-16-48 - v259 - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_CUSTOMLOG = compileFinal "
	...
	diag_log format['<infiSTAR.de>%1| %2   [22-05-2018 16-16-48 - v259]',_logname,_logentry];
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['a','8','e','7','0','f','2','e','6','9','c','c','3','c','d','2','a','8','e','a','9','2','3','e','7','8','2','4','c','7','0','0','6'];
...
if(_option isEqualTo 0)exitWith
{
	...
	_vehObj setdir _dir;
	_vehObj setVariable ['spawnedByAdmin',true,true];
...
if(_option isEqualTo 15)exitWith
{
	...
	if(_value)then
	{
		...
		[_array select 1] remoteExec ['RRPClient_system_restrain',_unit];
	}
	else
	{
		_unit setVariable['restrained',_value,true];
...
_log call FNC_A3_CONNECTLOG;
_m = missionNameSpace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=16982';missionNameSpace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
"; }; _A3AHstring = _A3AHstring + "
		...
		if(_add_Id > -1)then
		{
			...
			if(_announce)then
			{
				...
				[
					...
					'SpyGlass',
					'FFE4B5',
					format ['Player %1 (%2) changed Steam Accounts - UIDs: %3',name player,_puid,_PUID_array]
	...
	if(_isNormal)then {
		...
		[_name,_puid] spawn {
			...
			_addCaseHDMGold = -1;
			_addCaseMEHold = -1;
			if(isNil 'RRPClient_system_keyHandler')then{RRPClient_system_keyHandler = {false};}else{if(typeName RRPClient_system_keyHandler != 'CODE')then{RRPClient_system_keyHandler = {false};};};
			...
			_actionTitle = toLower _actionTitle;
			{
				...
				"; if(_RDK)then{ _A3AHstring = _A3AHstring + "
					...
					"; if(_MOD == 'AltisLife')then{ _A3AHstring = _A3AHstring + "
						...
						_kdID = (findDisplay 46) displayAddEventHandler ['KeyDown','_this call RRPClient_system_keyHandler;'];
					"; }; _A3AHstring = _A3AHstring + "
					"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
						_shouldbe = _shouldbe + 1;
					...
					if(!isNull _x)then
					{
						...
						if(_strx in _badIDDsToKick)then
						{
							...
							[
								...
								'SpyGlass',
								'FF0000',
								format ['BadDisplay: %1::: %2 (%3)',_strx,_name,_puid]
							] remoteExec ['RRPServer_discord_sendMessage',2];
							[_name,_puid,'HLOG_SKICK',toArray(_log)] call "+_AHKickLog+";
							call "+_AHKickOFF+";
						}
...
"; }; _A3AHstring = _A3AHstring + "
			...
			while{1==1}do
			{
				...
				"; if(_MIC)then{ _A3AHstring = _A3AHstring + "
					if((_twelvewasactive) && (!isNull (findDisplay 12 displayCtrl 51)))then
					{
						...
						{
							if((("+str _MOD+" == 'AltisLife')&&(player call getSide != 'cop'))||("+str _MOD+" != 'AltisLife'))then
							{
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '33964x2';
	_devLog = format['<infiSTAR.de> %1 VERSION: infiSTAR.de 22-05-2018 16-16-48 (v259) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_res = [VERSION_CHECK_URL_FULL+'&id=6c4861059ed8&li=8e70f2e69cc3cd2a8ea923e7824c7006&pr=A3'] call FN_ARMA_FETCHDATA;
...
_zeroCode = {
	if(hasInterface)then
	{
		_AHKickOFF = compileFinal '[] spawn {scriptName ''KICKOFF'';uiSleep 0.1;(findDisplay 46)closeDisplay 0;if(!isNil''"+_AHKickLog+"'')then{[profileName,getPlayerUID player,''KICKME''] call "+_AHKickLog+";};};';
...
_b = {[] spawn {uiSleep 20;waitUntil {time > 20};diag_log "<infiSTAR.de> license is blocked.";diag_log "<infiSTAR.de> if you did not do anything wrong. please open a support ticket on http://support.infiSTAR.de";diag_log "<infiSTAR.de> infiSTAR will NOT START when the license is blocked!";[] spawn {uiSleep 1;failMission "LOSER";endMission "LOSER";forceEnd;};if(!isNil"FN_GET_SERVERPW")then{(call FN_GET_SERVERPW) serverCommand "#shutdown";};};};
