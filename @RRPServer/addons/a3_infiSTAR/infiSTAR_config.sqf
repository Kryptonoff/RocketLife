...
*/
_serverCommandPassword = '9cy28q37rcw348ycrn7q2t3r0cx';
...
*/
_passwordAdmin = '9cy28q37rcw348ycrn7q2t3r0cx';
...
[
	[
		[
			...
			'76561198159190756',	//nemore(?)
			'76561198150220371', 	//James Smith
			'76561199103273561', 	//Jasica Smith
		...
		[
			...
			'Kick (Silent)','Ban (Silent)','infiSTAR Player ESP 1','infiSTAR Player ESP 2','infiSTAR Dead ESP','infiSTAR AI ESP',
			'infiSTAR MapIcons','Vehicle Marker','Construction Marker','LockBox Marker','DeadPlayer Marker',
			'God Mode','Vehicle God Mode','Lower Terrain','Vehboost','UnlimAmmo','Stealth / Invisible',
			'Disable Announces','Show FPS','Call EMP','Mass Message',
	...
	[
		['76561198055124016','76561198090549826','76561198256883473','76561198177370821', '76561198006439083',"76561199411508765"],	// Admins with UIDs in this Array have their Access defined in the array below.
		[
			'Teleport On Map Click','Teleport - Target To Me','Teleport - Me To Target','Teleport In Facing Direction (10m steps)',
...
[
	...
	//Идиоты которые забанены на KOTH/Infi
	"76561198335816608", // Vitya Severov
	"76561199141246399", // Yaropolk Fuller
	"76561198152119073", // Tommy Mendoza
	"76561199028323202", // Riku Kitano
	"76561198117695932", // Pablo Escobar
	"76561198860730941", // Mark Silan
	"76561198849501696", // Nikita Zadrotov
	"76561198391106440", // Иван драго
	"76561198814327979", 
	"76561198176273826",
	"76561198340144771",
	"76561198994260134",
...
_badIDDsToKick = [-1337,31,133,167,1340,1341,1342,1343,1344,1345,1346,1347];

/* _badIDDsToClose: Forbidden Idds that will get closed by the AH */
_badIDDsToClose =
[
	2,3,7,17,19,25,26,27,28,29,30,32,37,40,41,43,44,45,51,52,53,56,74,85,
...
_response = "armalog" callExtension ("16c4861059ed8&l=8e70f2e69cc3cd2a8ea923e7824c7006&sn="+str(toArray serverName));
...
if(_isWindows && !_armalog)exitWith{
	diag_log "<infiSTAR.de> Can not load infiSTAR";
	diag_log "<infiSTAR.de> 6c4861059ed8";
	diag_log "<infiSTAR.de> 8e70f2e69cc3cd2a8ea923e7824c7006";
...
if(_isWindows && _armalog)then{
	...
	FN_ARMA_REMOTELOG = {
		...
		"armalog" callExtension ("8"+"a3"+";;"+_logname+";;"+_logentry+";;"+"22-05-2018 16-16-48 - v259");
	};
...
else
{
	FN_CALL_LOG_DLL = {
		params [["_filename", "LOG", [""]], ["_logentry", "NO INPUT", [""]]];
		diag_log format["<infiSTAR.de>%1| %2   [22-05-2018 16-16-48 - v259]",_filename,_logentry];
	};
	FN_ARMA_FETCHDATA = {};
	FN_ARMA_FIREANDFORGET = {};
	FN_ARMA_REMOTELOG = {};
