...
fnc_lbDebugChanged = {
	...
	(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1) ctrlSetText (['infiSTAR_SAVED_FUNCTIONS',[]] call fnc_getPNS select _index);
};
fnc_fillDebugListBox = {
	_listbox = ((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1384;
...
fnc_getMainText = {
	private _txt = ctrlText(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339);
...
fnc_debug_loadbtn = {
	_var = ['infiSTAR_SAVED_FUNCTIONS',[]] call fnc_getPNS;
	_varname = lbData[1384,(lbCurSel 1384)];
	if(_varname isEqualTo '')then
	{
		'You need to select a function to load it..!' call chat;
	}
	else
	{
		_varvalue = [format['infiSTAR_SAVED_FUNCTION_%1',_varname],''] call fnc_getPNS;
		(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText _varvalue;
		
		format['Loaded: %1',_varname] call chat;
...
fnc_workplace = {
	...
	disableSerialization;
	if(isNull (uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull]))then{createdialog "infiSTAR_EDITBOX2";'AdminConsole' call fnc_adminLog;};
	_display = (uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull]);
	_buttonheight = 0.05;
	...
	_wf1cb1 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 7564982345) ctrlSetText ctrlText((findDisplay 46) displayCtrl 1362);true'];
	...
	_wf1cb2 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1380) ctrlSetText '''';[''wf1tTCT'',''''] call fnc_setPNS;true'];
	...
	_wf2cb1 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 7564982345) ctrlSetText ctrlText((findDisplay 46) displayCtrl 1262);true'];
	...
	_wf2cb2 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1381) ctrlSetText '''';[''wf2tTCT'',''''] call fnc_setPNS;true'];
	...
	_wf3cb1 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 7564982345) ctrlSetText ctrlText((findDisplay 46) displayCtrl 1162);true'];
	...
	_wf3cb2 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1382) ctrlSetText '''';[''wf3tTCT'',''''] call fnc_setPNS;true'];
	...
	_wf4cb1 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 7564982345) ctrlSetText ctrlText((findDisplay 46) displayCtrl 1062);true'];
	...
	_wf4cb2 ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1383) ctrlSetText '''';[''wf4tTCT'',''''] call fnc_setPNS;true'];
	...
	workplacemainwindowT = [] spawn {
		...
		while{true}do
		{
			_ctrl = ((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339;
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','[call fnc_getMainText] call admin_d0;''RAN CODE ON: GLOBAL (SERVER AND ALL CLIENTS)'' call chat;true'];
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','[''if(!isServer)then{''+(call fnc_getMainText)+''};''] call admin_d0;''RAN CODE ON: ALL BUT SERVER'' call chat;true'];

	_ctrlid = _ctrlid + 1;
	_ctrl = [_display,'RSCButton',_ctrlid] call fnc_createctrl;
	_ctrl ctrlSetText 'HEADLESS';
	_ctrl ctrlSetPosition [0,_startYpos + 1.1 + _buttonheight,0.25,_buttonheight];
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick',"if !(isNull hc_1) then
		{
			[call fnc_getMainText,hc_1] call admin_d0_target;
			'RAN CODE ON: HEADLESS CLIENT' call chat;
		};
		true
	"];
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','[call fnc_getMainText] call admin_d0_server;''RAN CODE ON: SERVER'' call chat;true'];
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','if(isPlayer SELECTED_TARGET_PLAYER)then{[call fnc_getMainText,SELECTED_TARGET_PLAYER] call admin_d0_target; format[''RAN CODE ON: %1'',name SELECTED_TARGET_PLAYER] call chat;};true'];
	};
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','(call fnc_getMainText) call fnc_admin_cc; ''RAN CODE ON: LOCAL'' call chat;true'];
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','if(!isNull cameraOn)then{[call fnc_getMainText,cameraOn] call admin_d0_target; format[''RAN CODE ON: %1'',name cameraOn] call chat;};true'];
	};
	...
	_ctrl ctrlSetEventHandler['ButtonClick','
		...
		if(_new <= _max)then
		{
			...
			(((uiNamespace getVariable [infiSTAR_EDITBOX2, displayNull])) displayCtrl 1339) ctrlSetText _txt;
		};
		format[''console: %1 / %2'',last_selected_run_id,_max] call chat;
	true
	'];
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText str ALLC_ITEMS;true'];
	};
	...
	_ctrl ctrlSetPosition [1,_buttonYpos,0.25,_buttonheight];
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','_txt = str ALL_VEHS_TO_SEARCH_C;(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText _txt;diag_log _txt;true'];
	};
	...
	_ctrl ctrlSetPosition [1,_buttonYpos,0.25,_buttonheight];
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','_txt = typeOf cursorTarget;(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText _txt;diag_log _txt;true'];
	...
	_ctrl ctrlSetPosition [1,_buttonYpos,0.25,_buttonheight];
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','_txt = typeOf vehicle player;(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText _txt;diag_log _txt;true'];
	...
	_ctrl ctrlSetPosition [1,_buttonYpos,0.25,_buttonheight];
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','_txt = str(getPos player);(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText _txt;diag_log _txt;true'];
	...
	_ctrl ctrlSetPosition [1,_buttonYpos,0.25,_buttonheight];
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','_txt = str(getPosATL player);(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText _txt;diag_log _txt;true'];
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1339) ctrlSetText '''';[''workplacemainwindow'',''''] call fnc_setPNS;true'];
	...
	_ctrl ctrlCommit 0;
	_ctrl ctrlSetEventHandler['ButtonClick','
		_varname = ctrlText(((uiNamespace getVariable ["infiSTAR_EDITBOX2", displayNull])) displayCtrl 1);
		if(_varname isEqualTo '''')then
		{
			''You need to write a name to save this function..!'' call chat;
		}
		else
		{
			_var = [''infiSTAR_SAVED_FUNCTIONS'',[]] call fnc_getPNS;
			...
			format[''Saved: %1'',_varname] call chat;
			call fnc_fillDebugListBox;
		};
	true
	'];
	...
	_ctrl ctrlSetEventHandler['ButtonClick','
		...
		if(_varname isEqualTo '''')then
		{
			''You need to select a function to delete..!'' call chat;
		}
		else
		{
			if(isNil''last_delete_call'')then
			{
				...
				format[''Please click again to delete: %1'',_varname] call chat;
			}
			else
			{
				if(last_delete_call isEqualTo _varname)then
				{
					...
					format[''Deleted: %1'',_varname] call chat;
					call fnc_fillDebugListBox;
