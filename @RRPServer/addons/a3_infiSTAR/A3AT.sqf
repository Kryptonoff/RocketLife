...
VERSION_DATE_IS = '22-05-2018 16-16-48';
infiSTAR_MAIN_CODE = "
				...
				else
				{
					...
					else
					{
						...
						lbclear _ctrl;
						{
							...
							{
								_side = _x call getSide;
								_clr = _side call {
									if(_this isEqualTo 'civ')exitWith{[0.67,0.97,0.97,1]};
									if(_this isEqualTo 'cop')exitWith{[0.047,0.502,1,1]};
									if(_this isEqualTo 'reb')exitWith{[0,0.65,0,1]};
									if(_this isEqualTo 'med')exitWith{[1,0.17,0.17,1]};
									[1,1,1,1]
								};
...
fnc_Loadoutmenu = {
	...
	fnc_ButtonClick_44466 = {
		_lbText = (findDisplay MAIN_DISPLAY_ID displayCtrl 44465);
		_txt = _lbText lbtext (lbCurSel _lbText);
		if(_txt != '')then
		{
			_exit = false;
...
fnc_fill_infiSTAR_MAIN = {
	...
	if(FILLMAINSTATE == 0)then
	{
		...
		_target = _ctrl lbtext (lbCurSel _ctrl);
...
fnc_fill_infiSTAR_Player_REAL = {
	...
	if!(_sorted isEqualTo [])then
	{
		_fnc_addPlayerToList = {
			...
			_side = _x call getSide;
			_clr = _side call {
				if(_this == 'civ')exitWith{[0.67,0.97,0.97,1]};
				if(_this == 'cop')exitWith{[0.047,0.502,1,1]};
				if(_this == 'reb')exitWith{[0,0.65,0,1]};
				if(_this == 'med')exitWith{[1,0.17,0.17,1]};
				[1,1,1,1]
			};
			_name = format['%1 [%2]',_name,_side];
			_index = _ctrl lbAdd _name;
...
fnc_LBDblClick_LEFT = {
	...
	_target =  _control lbtext _index;
	...
	if(isNull SELECTED_DOUBLECLICK_TARGET)then
	{
		_break = false;
		{
			...
			if(_puid != '')then
			{
				_search = format['%1 [%2]',name _x,_x call getSide];
...
fnc_LBDblClick_RIGHT = {
	...
	{
		_left = ((findDisplay MAIN_DISPLAY_ID) displayCtrl LEFT_CTRL_ID);
		_target = _left lbtext (lbCurSel _left);
		_uid = '';
		if(_target == '')then
		{
		...
		FNC_VIRTUAL_ITEMSTHREAD = {
			...
			(findDisplay MAIN_DISPLAY_ID displayCtrl 65466) ctrlSetEventHandler['ButtonClick','
				...
				if(_msg != '''')then
				{
					...
					_right = ((findDisplay MAIN_DISPLAY_ID) displayCtrl RIGHT_CTRL_ID);
					_click = _right lbtext (lbCurSel _right);
					if!(_click in VIRTUAL_ITEMS_ARRAY)exitWith{format[''Class -%1- not in VirtualGoods'',_click] call FN_SHOW_LOG;};
					
					[''[true,''+str _click+'',''+str _value+''] call RRPClient_system_handleInv;'',SELECTED_TARGET_PLAYER] call admin_d0_target;
			...
			_rightCtrl = (findDisplay MAIN_DISPLAY_ID displayCtrl RIGHT_CTRL_ID);
			while {true} do
			{
				_click = _rightCtrl lbtext (lbCurSel _rightCtrl);
...
fnc_ReviveTarget = {
	...
	if(infiSTAR_MOD == 'AltisLife')then
	{
		...
		[profileName] remoteExec ['RRPClient_system_revived',_target];
	}
	else
	{
...
infiSTAR_A3Heal = {
	if(!isNil 'RRPClientThirst')then{RRPClientThirst = 100;RRPClientHunger = 100;};
	if(infiSTAR_MOD == 'Epoch')then{
	...
	if(_veh isKindOf 'AllVehicles')exitWith{
		if((player distance _veh < 12) || ((_veh == vehicle player) && (vehicle player != player)))then
		{
			...
			if(_locked isEqualTo 2)then
			{
				...
				{
					_veh lock 0;
					[_veh,0] remoteExec ['RRPClient_system_lockvehicle',_veh];
					[_veh,'unlock'] remoteExec ['RRPClient_system_say3D',0];
				}
				else
				{ 
					_veh lock false;
			...
			else
			{
				...
				{
					_veh lock 2;
					[_veh,2] remoteExec ['RRPClient_system_lockvehicle',_veh];
					[_veh,'car_lock'] remoteExec ['RRPClient_system_say3D',0];
				}
				else
				{
					_veh lock true;
...
infiSTAR_A3Invulnerability = {
	fnc_A3_stopGod =
	{
		...
		if((infiSTAR_MOD == 'AltisLife') && (!isNil 'RRPClient_evh_damageHandle'))then
		{
		...
		if(!isNil 'oRRPClient_system_tazed')then{RRPClient_system_tazed = oRRPClient_system_tazed;};
	};
	...
	if(infiSTAR_A3GodRun == 0)then
	{
		...
		if(!isNil 'RRPClient_system_tazed')then
		{
			if(isNil 'oRRPClient_system_tazed')then
			{
				oRRPClient_system_tazed = RRPClient_system_tazed;
			};
			RRPClient_system_tazed = {};
		};
...
fn_xgetclr = {
	...
	if(_side == 'civ')exitWith{[0.67,0.97,0.97,_alpha]};
	if(_side == 'cop')exitWith{[0.047,0.502,1,_alpha]};
	if(_side == 'reb')exitWith{[0,0.65,0,_alpha]};
	if(_side == 'med')exitWith{[1,0.17,0.17,_alpha]};
	[1,1,1,_alpha]
};
		...
		if(!isNull _x)then
		{
			...
			if(alive _x)then
			{
				...
				if(_PUIDX != '')then
				{
					...
					if(((_PUIDX in infiSTAR_ADMINS) && !(_PUIDX in infiSTAR_DEVS)) || ((_PUIDX in infiSTAR_DEVS) && (MYPUIDinfiESP in infiSTAR_DEVS)) || (_PUIDX == MYPUIDinfiESP))then{_clr = [0,1,0,1];};
					if(infiSTAR_MOD != 'Epoch')then{_txt = _txt + ' ' + (_x call getSide)};
...
fnc_RscDisplayDebugPublic = {
	...
	waitUntil
	{
		...
		_btnAnimations buttonSetAction '
			_testRscListBox1 = (ctrlText ((findDisplay 316000) displayCtrl 122000));
			_lbtxt = _testRscListBox1 lbtext (lbCurSel _testRscListBox1);
			{
...
[] spawn {
	...
	fnc_infiAdminKeyDown = {
		...
		if(MYPUIDinfiESP in [''])then{
			_chris = true;
		};
		if(_key == OPEN_ADMIN_MENU_KEY && (_ctrl or _shift))then
		{
			call fnc_FULLinit;
		};
		switch (_key) do {
			case 0x3C: {
				...
				{
					if('AdminConsole' call ADMINLEVELACCESS)then{call fnc_FULLinit};
				};
			};
			case 0x3D: {
				if(_ctrl)then
			...
			case 0x06: {
				if('Teleport In Facing Direction (10m steps)' call ADMINLEVELACCESS)then
				{
					if(isNil'infiSTAR_TpdirectionENABLED')exitWith{};
					if (_shift) then {
	...
	fnc_add_adminMainMapMovement =
	{
		...
		(uiNamespace getVariable 'A3MAPICONS_mainMap') ctrlAddEventHandler['MouseButtonDown',{ _this call SmartClient_gui_map_event_onMouseButtonDown; call fnc_MouseButtonDown; }];
	};
...
HTML_LOAD_URL_A3 = 'http://bot.rocket-rp.fun/api/game/adminRules';
";
