...
try
{
	...
	[format ["Old Balance Player: %7 (%8): CASH (%1) ATM (%2) POINT (%3) DCASH (%4) - mode: %5 Price: %6",CTEXT(_cash),CTEXT(_atm),CTEXT(_point),CTEXT(_dcash),_mode,CTEXT(_price),name _player,_playerUID],"MoneyChange"] call RRPServer_system_logIt;
	switch (_type) do
	{
		case "cash":
		{
		...
		case "point":
		{
			switch (_mode) do
			{
				...
				case "take": 
				{
					if (_price < 0) then
					{
						[_playerUID,999999,"Пополнять RC нужно было через ЛК! Сорян:)"] call RRPServer_DB_addBan;
					};
					_point = floor (_point - _price);
					_player setVariable ["RRPPoint",_point];
				};
		...
		case "trx":
		{
			switch (_mode) do
			{
				...
				case "set": 
				{
					_player setVariable ["RRPTRX",_price, true];
				};
				default {};
			};
		};
		case "serverbank":
		{
			...
			{
				case "add": {
					format ["ADD MONEY CHANGE SERVER BANK %1 - BALANCE %2 #1111",_price, server_bank, server_bank + _price] call RRPServer_util_debugConsole;
					server_bank = server_bank + _price;
					publicVariable "server_bank";
					format ["serverBank:%1",server_bank] call RRPServer_DB_fireAndForget;
				};
				case "take": {
					format ["TAKE MONEY CHANGE SERVER BANK %1 - BALANCE %2 | NEW BALANCE %3 #1111",_price, server_bank,server_bank - _price] call RRPServer_util_debugConsole;
					server_bank = server_bank - _price;
					publicVariable "server_bank";
					format ["serverBank:%1",server_bank] call RRPServer_DB_fireAndForget;
				};
				case "set": {
					...
					publicVariable "server_bank";
					format ["serverBank:%1",server_bank] call RRPServer_DB_fireAndForget;
					format ["SET MONEY CHANGE SERVER BANK %1 #1111",_price] call RRPServer_util_debugConsole;
				};
				default {};
			};
		};
		default {throw format["валюта не определена: %1",_type]};
	};
	_cash = _player getVariable ["RRPCash",0];
	_atm = _player getVariable ["RRPAtm",0];
	_point = _player getVariable ["RRPPoint",0];
	_dcash = _player getVariable ["RRPDcash",0];
	_trx = _player getVariable ["RRPTRX",0];
	[format ["New Balance Player: %7 (%8): CASH (%1) ATM (%2) POINT (%3) DCASH (%4) - mode: %5 Price: %6",CTEXT(_cash),CTEXT(_atm),CTEXT(_point),CTEXT(_dcash),_mode,CTEXT(_price),name _player,_playerUID],"MoneyChange"] call RRPServer_system_logIt;
	format["updateMoney:%1:%2:%3:%4:%5:%6",_cash,_atm,_point,_dcash,_trx,_playerUID] call RRPServer_DB_fireAndForget;
	[_cash,_atm,_point,_dcash,_trx] remoteExecCall ["RRPClient_system_syncMoney",_player];
}catch{
