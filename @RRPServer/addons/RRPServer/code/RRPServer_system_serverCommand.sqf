...
switch (_command) do {
	...
	//case "message": { [0,"BattleEye Client: (Global) Admin: asdasdasd"] remoteExecCall ["RRPClient_system_broadcast",-2]; };
	...
	case "ddosts": {
		if (server_ddos_ts) then 
		{
			...
			[0,"Внимание! На TS3 сервере завершились магнитные бури."] remoteExecCall ["RRPClient_system_broadcast",-2];
		} else {
			server_ddos_ts = true;
			publicVariable "server_ddos_ts";
			[0,"Внимание! На TS3 сервере наблюдаются магнитные бури. Возможны перебои со связью."] remoteExecCall ["RRPClient_system_broadcast",-2];
		};
	};
	case "ddosserver": {
		if (server_ddos_air) then 
		{
			...
			[0,"Внимание! На острове окончились магнитные бури. Воздушная техника доступна."] remoteExecCall ["RRPClient_system_broadcast",-2];
		} else {
			server_ddos_air = true;
			publicVariable "server_ddos_air";
			[0,"Внимание! На острове наблюдаются магнитные бури. Воздушная техника временно недоступна."] remoteExecCall ["RRPClient_system_broadcast",-2];
		};
	};
};
