/*
	...
	Filename:	RRPClient_hotkeys_clickToLeftMenuButton.sqf
...
if (_display getVariable ["load_status",false]) exitWith {};
...
switch (_button_name) do {
	...
	case "weapons": {
		_array_weapons = ["hotkey_primary", "hotkey_secondary", "hotkey_handgun", primaryWeapon player, secondaryWeapon player, handgunWeapon player] - [""];
		{
			if (_finaly_array isEqualTo []) then {
				_finaly_array pushBack [_x,1];
			} else {
	...
	case "gesture": {
		...
		for "_i" from 0 to count(_cfg)-1 do {
			...
			_finaly_array pushBack [_action,0]
...
_array_map_delete = +[];
{
	...
	switch (_button_name) do {
		case "weapons": {
			switch (_items) do { 
				...
				default {
					_itemInfo = [_items] call RRPClient_util_itemDetails;
	...
	_grpLMP_button ctrlSetEventHandler ["MouseButtonDown",format ['[%1,"down",(_this select 0)] call RRPClient_hotkeys_mouseButtonHandler',[_button_name,_items,_forEachIndex,_countItems]]];
	_grpLMP_button ctrlSetEventHandler ["MouseButtonUp",format ['[%1,"up",(_this select 0)] call RRPClient_hotkeys_mouseButtonHandler',[_button_name,_items,_forEachIndex,_countItems]]];
	...
	switch (_button_name) do {
		case "inventory": {
			[((format ["left_group_items_text_count_%1",_forEachIndex]) call _getNextIDC),_grpLMP_button,[0.005,0,0.05,_height],format["x%1",_countItems],1.3,"RscText"] call _makeTextControl;
		};
		case "gesture";
		case "dance";
		case "pose": {
			[((format ["left_group_items_text_count_%1",_forEachIndex]) call _getNextIDC),_grpLMP_button,[0.005,0,0.05,_height],/*format["VL-%1",_countItems]*/"",1,"RscText"] call _makeTextControl;
		};
		default {
			/* code */
		};
	...
	[((format ["left_group_items_text_name_%1",_forEachIndex]) call _getNextIDC),_grpLMP_button,[0.05 + 0.005 + 0.01 + (_height - 0.03),0,(_widthLMP - (0.05 + 0.005 + 0.01 + (_height - 0.03))),_height],_name,1.3,"RscText"] call _makeTextControl;
