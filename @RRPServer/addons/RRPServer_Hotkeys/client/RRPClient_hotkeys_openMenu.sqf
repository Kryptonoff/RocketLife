/*
	...
	Filename:	RRPClient_hotkeys_openMenu.sqf
...
[false] call RRPClient_hotkeys_hudWeaponHide;

//settings
private _pictureGesture = getText(missionConfigFile >> "LifeCfgSettings" >> "MFMpictureGesture");
private _pictureDance = getText(missionConfigFile >> "LifeCfgSettings" >> "MFMpictureDance");
private _picturePose = getText(missionConfigFile >> "LifeCfgSettings" >> "MFMpicturePose");
private _picturePlus = "\rimas_agent\assets\hotkeys\icon_more.paa";
private _pictureMinus = "\rimas_agent\assets\hotkeys\icon_less.paa";
...
//Setup eventhandlers
_display displayAddEventHandler ["unload","removeMissionEventHandler ['Draw3D',((_this select 0) getVariable ['drawEH',-1])]; if (life_showWeaponHud) then {[true] call RRPClient_hotkeys_hudWeaponHide} else {[false] call RRPClient_hotkeys_hudWeaponHide}; [] call RRPClient_hotkeys_hudWeaponUpdate; call fsc_profile_saveSettings;"];
_display setVariable ["drawEH", (addMissionEventHandler ["Draw3D",{_this call RRPClient_hotkeys_menuOnDraw}])];
...
for "_i" from 0 to 16 do {
	...
	if (_i >= 9) then {
		_button_text = format ["F%1",(_i - 8)];
		_posX = (0.34 * safezoneW + safezoneX) + ((_space * (_i - 9)) + (_width * (_i - 9)));
		_posY = 0.848 * safezoneH + safezoneY
	} else {
	...
	switch (_type_name) do {
		case "weapons": {
			switch (_item) do { 
				case "hotkey_primary" : {
					_picture = "\rimas_agent\assets\hotkeys\icon_primary.paa";
				}; 
				case "hotkey_secondary" : {
					_picture = "\rimas_agent\assets\hotkeys\icon_secondary.paa";
				}; 
				case "hotkey_handgun" : {
					_picture = "\rimas_agent\assets\hotkeys\icon_handgun.paa";
				}; 
				default {
...
_button_del_f_number = [("button_del_1" call _getNextIDC),[0.263 * safezoneW + safezoneX,0.848 * safezoneH + safezoneY,0.07 * safezoneW,0.044 * safezoneH],"[""f_nubmer"",[]] call RRPClient_hotkeys_deleteBind;","Сбросить ->",0] call _makeButtonNotParentControl;
_button_del_number = [("button_del_2" call _getNextIDC),[0.263 * safezoneW + safezoneX,0.918 * safezoneH + safezoneY,0.07 * safezoneW,0.044 * safezoneH],"[""number"",[]] call RRPClient_hotkeys_deleteBind;","Сбросить ->",0] call _makeButtonNotParentControl;
...
private _left_menu_title_array = [["Инвентарь","inventory"],["Оружие","weapons"],["Анимации - жесты","gesture"],["Анимации - танцы","dance"],["Анимации - позы","pose"]];
...
for "_i" from 0 to (count _left_menu_title_array) - 1 do {
	...
	[((format ["left_button_text_%1",_i]) call _getNextIDC),_grp,[0.04 + (_heightLMB - 0.05),0.01,(_widthLMB - 0.05 - (_heightLMB - 0.05)),_heightLMB - 0.02],((_left_menu_title_array select _i) select 0),2,"RscText"] call _makeTextControl;

	[((format ["left_button_%1",_i]) call _getNextIDC),_grp,[0,0,_widthLMB, _heightLMB],"",format ["['%1'] spawn RRPClient_hotkeys_clickToLeftMenuButton",((_left_menu_title_array select _i) select 1)]] call _makeButtonControl;
...
['inventory'] spawn RRPClient_hotkeys_clickToLeftMenuButton;
