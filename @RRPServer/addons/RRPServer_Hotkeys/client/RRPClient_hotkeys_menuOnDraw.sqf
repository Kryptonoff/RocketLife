/*
	...
	Filename:	RRPClient_hotkeys_menuOnDraw.sqf
...
if (_isButtonClicked AND !_isButtonGrabbed) then {
	...
	if !(isNull _realBut) then {
		...
		switch (_button_info select 0) do {
			case "weapons";
			case "inventory": {
				[("buttonGrabbedFakeTextCount" call _getNextIDC),_buttonGrabbedFakeGrp,[0.005,0,0.05,_height],format["x%1",_countItems],1.3,"amazing_tabletButtonText"] call _makeTextControl;
			};
			case "gesture";
			case "dance";
			case "pose": {
				[("buttonGrabbedFakeTextCount" call _getNextIDC),_buttonGrabbedFakeGrp,[0.005,0,0.05,_height],format["VL-%1",_countItems],1,"amazing_tabletButtonText"] call _makeTextControl;
			};
			default {
				/* code */
			};
			...
			[("buttonGrabbedFakeTextTitle" call _getNextIDC),_buttonGrabbedFakeGrp,[0.05 + 0.005 + 0.01 + (_height - 0.03),0,(_width - (0.05 + 0.005 + 0.01 + (_height - 0.03))),_height],_name,1.3,"ATT_left"] call _makeTextControl;
	...
	for "_i" from 0 to 16 do {
		...
		if (((_xm>_xp) AND (_xm<(_xp+_w))) AND ((_ym>_y) AND (_ym<(_y+_h)))) then {
			if !((_display getVariable ["advInfo",-1]) isEqualTo _i) then {
				...
				_arrayInfo = ["",""];
				if (_i <= 8) then {
					_arrayInfo = (bind_number_array select _i);
				} else {
				...
				if (!(_type_name isEqualTo "") && !(_item isEqualTo "")) then {
					...
					if (isNull (missionNamespace getVariable ["MFM_adv_info_handler",objNull])) then {terminate MFM_adv_info_handler;};
