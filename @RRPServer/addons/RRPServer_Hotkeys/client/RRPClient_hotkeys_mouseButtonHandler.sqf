/*
	...
	Filename:	RRPClient_hotkeys_mouseButtonHandler.sqf
	...
	if (_isButtonGrabbed) then {
			...
			if (((_xm>_xp)AND(_xm<(_xp+_w)))AND((_ym>_y)AND(_ym<(_y+_h)))) then {
				...
				if (_notAllowed) exitWith {
					...
					MFM_adv_info_handler = [_adv_info,_display] spawn {
						...
						_adv_info ctrlSetStructuredText parseText "<t size='1' align='center'><br/>Бинд недоступен<br/><br/></t>";
				...
				switch (true) do {
					case (_i <= 8): {
						bind_number_array set [_i,[_type_name,_item]];
					};
					default {
...

		{
			...
			switch (_button_info select 0) do {
			    case "gesture": {
			      [(_button_info select 1)] call RRPClient_system_animDo;
			    };
			    case "dance": {
			      [(_button_info select 1)] spawn RRPClient_gestures_danceAction;
			    };
			    case "pose": {
			      [(_button_info select 1)] spawn RRPClient_gestures_playAction;
			    };
			    default {
			      /* code */
			    };
