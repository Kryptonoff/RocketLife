...
#define DCASH RRPClientCashMoneyD
#define CASH RRPClientCashMoney
#define BANK RRPClientAtmMoney
#define SmartPoints RRPClientCredits
#define VAR(NAME) RRPClient##NAME
...
#define LOADING_TEXT(var) var call RRPClient_gui_loadingScreen_updateText;var call RRPClient_util_log;
...
#define CHECKSYMB(var) _alph = getText(missionConfigFile >> "LifeCfgSettings" >> "allowAlphabet");_index = [var,_alph] call RRPClient_util_containsForbiddenCharacter;if !(_index isEqualTo -1) throw (format ["Нельзя использовать '%1'!",_index]);
#define CHECKNUMB(var) _alph = getText(missionConfigFile >> "LifeCfgSettings" >> "allowNumber");_index = [var,_alph] call RRPClient_util_containsForbiddenCharacter;if !(_index isEqualTo -1) throw (format ["Нельзя использовать '%1'!",_index]);
...
#define ISPSIDE(condition1) (player call getSide) isEqualTo condition1
...
//ВОТ ЭТО ВООБЩЕ ПРЯМ ВАЖНО!
#define ITEM_CANSELL(varName) M_CONFIG(getNumber,"VirtualItems",varName,"canSell")
#define ITEM_INVCANSELL(varName) M_CONFIG(getNumber,"LifeCfgItems",varName,"canSell")


#define LICENSE_VARNAME(varName,flag) format ["license_%1",M_CONFIG(getText,"LifeCfgLicenses",varName,"variable")]
