/*
	...
	Filename: 	RRPClient_system_removeWeapons.sqf
...
if (!(EQUAL(_pWeapon,"")) && {!(_pWeapon in _whitelist)}) then {
	...
	_ItemInfo = [_pWeapon] call RRPClient_util_itemDetails;
	format["%1, ваше нелегальное оружие (%3) изъято сотрудником полиции %2", GVAR_RNAME(player), GVAR_RNAME(_cop), _ItemInfo select 1] call chat;
};
...
if (!(EQUAL(_hWeapon,"")) && {!(_hWeapon in _whitelist)}) then {
	...
	_ItemInfo = [_hWeapon] call RRPClient_util_itemDetails;	
	format["%1, ваше нелегальное оружие (%3) изъято сотрудником полиции %2", GVAR_RNAME(player), GVAR_RNAME(_cop), _ItemInfo select 1] call chat;
};
...
if (!(EQUAL(_sWeapon,"")) && {!(_sWeapon in _whitelist)}) then {
	...
	_ItemInfo = [_sWeapon] call RRPClient_util_itemDetails;
	format["%1, ваше нелегальное оружие (%3) изъято сотрудником полиции %2", GVAR_RNAME(player), GVAR_RNAME(_cop), _ItemInfo select 1] call chat;
};
...
//удаляем нелегал
{
	if !(_x in _whitelist) then {
		if ([_x] call RRPClient_system_removeItem) then {
...
if (!(EQUAL(_vest,"")) && {!(_vest in _whitelist)}) then {
	removeVest player;	
	format["%1, ваша нелегальная разгрузка изъята сотрудником полиции %2", GVAR_RNAME(player), GVAR_RNAME(_cop)] call chat;
};

if (!(EQUAL(_uniform,"")) && {!(_uniform in _whitelist)}) then {
	removeUniform player;	
	format["%1, ваша нелегальная одежда изъята сотрудником полиции %2", GVAR_RNAME(player), GVAR_RNAME(_cop)] call chat;
};

if (_reward > 0) then {
	[player,_reward] remoteExecCall ["RRPClient_system_removeWeaponsReward",_cop];
};
