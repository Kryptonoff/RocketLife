/*
	...
	Filename: 	RRPClient_system_vehicleAnimate.sqf
...
switch (player call getSide) do {
	...
	case "civ": {
		if (EQUAL(_className,"B_Heli_Light_01_F") && !([LIFE_SETTINGS(getText,"life_benchesForCivHumm")] call RRPClient_util_conditionsCheck)) then {_animate = "civ_littlebird"};
		if (EQUAL(_className,"C_Heli_Light_01_civil_F") && (license_taxi)) then {_animate = "taxi_heli"};
	};
	case "reb": {
