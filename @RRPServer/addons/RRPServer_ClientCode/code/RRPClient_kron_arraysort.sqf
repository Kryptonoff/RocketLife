...
_a = +(_this select 0);
_d = if ([_this,"DESC"] call RRPClient_kron_findFlag) then {-1} else {1};
_k = if ([_this,"CASE"] call RRPClient_kron_findFlag) then {"CASE"} else {"nocase"};
_s = -1;
...
for "_i" from 0 to (count _a)-1 do {
	...
	for [{_j=_i-1},{_j>=0},{_j=_j-1}] do {
		...
		_c=[_v2,_v1,_k] call RRPClient_kron_Compare;
		if (_c!=_d) exitWith {};
