/*
	...
	Filename: 	RRPClient_system_houseTakeItem.sqf
...
try{
	...
	if (EQUAL((lbCurSel 4102),-1)) throw "Не выбрано";
	...
	if (_num <= 0) throw "Число больше нуля...";

	private _itemInfo = [_item] call RRPClient_util_itemDetails;
	if (EQUAL(_itemInfo,[])) exitWith {};
	if !([_item] call RRPClient_system_canUseItem) throw "Вы не можете использовать этот предмет";
	...
	if (EQUAL(_indexH,-1)) throw "У вас нет такого предмета в хранилище";
	if (EQUAL(_itemCaregory,"Equipment") && {(_num > 1)}) throw "Предметы одежды можно забирать только по одному";
	...
	if (EQUAL(_message,"")) then {
		["InfoTitleAndText",["Предмет взят из хранилища"]] call toast;
	} else {
		["InfoTitleAndText",[_message]] call toast;
	};
