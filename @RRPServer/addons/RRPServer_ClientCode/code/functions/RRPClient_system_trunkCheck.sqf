/*
	...
	Filename: 	RRPClient_system_trunkCheck.sqf
...
private _newTrunk = [];
{
	if (isClass (missionConfigFile >> "VirtualItems" >> (_x select 0))) then {
		...
		format ["Trunk Check - %1 is not Virtual Item. Removed from array",_x select 0] call RRPClient_system_log; 
	};
} foreach _trunk;
...
if (EQUAL(_newTrunk,[])) then {
	[_object,"Trunk"] call RRPClient_system_clearGlobalVar;
} else {
