/*
	...
	Filename: 	RRPClient_system_addItem.sqf
...
if (_itemInfo isEqualTo []) exitWith {
	diag_log format ["RRPClient_system_addItem |bad item: %1",_item];
};
...
switch (_category) do {
	...
	case "Weapon": {
		...
		if (_canAdd) then {
			...
			if (_item in ["Extremo_Tool_Shovel","Extremo_Tool_Axe"]) then {
			...
			_return = [_item] call RRPClient_system_addItemToInventory;
		};
	};
		...
		switch (_type) do {
			case "Glasses": {
				if (EQUAL(PGOOGLES,"")) then {
					...
					[PGOOGLES] call RRPClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
			...
			case "Headgear": {
				if (EQUAL(PHEADGEAR,"")) then {
					...
					[PHEADGEAR] call RRPClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
			...
			case "Vest": {
				if (EQUAL(PVEST,"")) then {
					...
					[PVEST] call RRPClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
					private _items = PVEST_ITEMS;
					removeVest player;
					player addVest _item;
					{
						[_x,"vest"] call RRPClient_system_addItemToInventory;
					} forEach _items;
					_return = true;
				};
			};
			case "Uniform": {
				if (EQUAL(PUNIFORM,"")) then {
					...
					[PUNIFORM] call RRPClient_system_addItemToInventory; //попробуем добавить текущий предмет в инвентарь
					private _items = PUNIFORM_ITEMS;
					removeUniform player;
					player forceAddUniform _item;
					{
						[_x,"uniform"] call RRPClient_system_addItemToInventory;
					} forEach _items;
					_return = true;
				};
			};
			case "Backpack": {
				if (EQUAL(PBACKPACK,"")) then {
					...
					clearAllItemsFromBackpack player;
					{
						[_x,"backpack"] call RRPClient_system_addItemToInventory;
					} forEach _items;
					_return = true;
				};
			};
	...
	case "Item": {
		switch (true) do {
			...
			case (_type in ["AccessoryMuzzle","AccessoryPointer","AccessorySights","AccessoryBipod"]) : {
				_return = [_item] call RRPClient_system_addItemToWeapon;
			};
			//карта, бинокль и т.д.
			...
			default {_return = [_item] call RRPClient_system_addItemToInventory};
		};
	};
	default {};
};
