/*
	...
	Filename: 	RRPClient_system_addItemToInventory.sqf
...
if (EQUAL(_to,"")) then {
	switch (true) do {
		...
		default {
        	...
        	_return = [_item,_weaponHolder] call RRPClient_system_addItemToBox;
        	if (_return) then {
        		"У вас в инвентаре нет свободного места, поэтому предмет был сброшен на землю! Не забудьте его подобрать, если Вы его не подберете, он удалится в ближайшее время!" call chat;
        	};
		    
			[_weaponHolder] spawn {uiSleep 300; deleteVehicle (_this select 0)};
		};
	};
} else {
	switch (_to) do {
