...
private ["_price","_condition"];
_price = -1;
_condition = "true";
if (_category isEqualTo "Equipment") then {
	_price = if (isClass (missionConfigFile >> "clothing_settings" >> _item)) then {
		getNumber(missionConfigFile >> "clothing_settings" >> _item >> "price")} else {-1};

	_condition = if (isClass (missionConfigFile >> "clothing_settings" >> _item)) then {
		getText(missionConfigFile >> "clothing_settings" >> _item >> "condition_sell")} else {""};

} else {
	_price = if (isClass (missionConfigFile >> "LifeCfgItems" >> _item)) then {
		getNumber(missionConfigFile >> "LifeCfgItems" >> _item >> "price")} else {-1};

	_condition = if (isClass (missionConfigFile >> "LifeCfgItems" >> _item)) then {
		getText(missionConfigFile >> "LifeCfgItems" >> _item >> "condition")} else {""};
};

private _return = [_item,_displayName,_picture,_desc,_category,_type,_weight,_magazines,_muzzles,_optics,_pointers,_bipods,_price,_condition];
