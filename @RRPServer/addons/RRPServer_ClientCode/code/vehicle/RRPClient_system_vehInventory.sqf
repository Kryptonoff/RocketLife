/*
	...
	Filename: 	RRPClient_system_vehInventory.sqf
...
switch (true) do {
	...
	default {
		_veh_data = [_veh] call RRPClient_system_vehicleWeight;
...
[_pInv] call RRPClient_gui_ListVirtualItems;
...
private "_icon";
{
	if !((_x select 0) in ["money","dmoney"]) then {
		...
		_icon = switch (_x select 0) do {
			case "money" : {"\rimas_pack\icons\items\money.paa"};
			case "dmoney" : {"\rimas_pack\icons\items\dmoney.paa"};
		};
		if ((_x select 1) > 0) then {
			_tInv lbAdd format["$%1",[_x select 1] call RRPClient_util_numberText];
