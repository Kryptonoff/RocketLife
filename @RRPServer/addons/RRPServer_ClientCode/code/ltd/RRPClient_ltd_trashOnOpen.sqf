...
private _storeDialog = uiNamespace getVariable ["RRPDialogLTDShop",displayNull];

closeDialog 7400;
createDialog "RRPDialogLTDModer";
...
}forEach [7502,7503,7504,7505,7506,7507,7508,7509,7510,7511,7512,7513,7514,7515];


private	_toStockButton = _display displayCtrl 7511;
...
_storeOwner = life_shop_npc getVariable ["ltdOwner",0];
if !(_storeOwner isEqualTo (RRPClientLTDsData select 0)) then 
{
	_toStockButton ctrlSetEventHandler ["ButtonClick","_this call RRPClient_ltd_getBackMyTrashRequest"];
	_toStockButton ctrlSetText "Забрать этот товар";
	{
		[_display, _x, ctrlPosition (_display displayCtrl _x), 1, 0] call RRPClient_gui_util_animateCtrl;
		ctrlEnable [_x,false];
	}forEach [7508,7514,7515];
};	
