...
try
{
	...
	if (EQUAL(_itemPrice,-1)) throw "Ошибка с ценой предмета";
	...
	if (_isVirtItem) then {
		if ([true,_itemClass,_itemCount] call RRPClient_system_handleInv) then {
			["InfoTitleOnly", [format["Вы купили %1",_itemName]]] call SmartClient_gui_toaster_addTemplateToast;	
		...
		["SuccessTitleOnly", [format["Вы купили %1",_itemName]]] call SmartClient_gui_toaster_addTemplateToast;
		life_action_inUse = false;
		call RRPClient_ltd_shopUpdate;
	};
}catch{
