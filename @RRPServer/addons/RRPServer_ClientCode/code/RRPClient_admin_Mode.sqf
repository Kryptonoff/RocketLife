/*
	...
	Filename: 	RRPClient_admin_mode.sqf
...
switch (life_admin_mode) do {
    case true: {
		...
		RRPClient_system_copEnter,"driver",200,false,false,"",
		...
		RRPClient_system_copEnter,"passenger",200,false,false,"",
		'!isNull cursorTarget && {((cursorTarget isKindOf "LandVehicle")||(cursorTarget isKindOf "Air")||(cursorTarget isKindOf "Ship"))} && {(locked cursorTarget) != 0}'
		]);
		...
		RRPClient_system_copEnter,"exit",200,false,false,"",
	   ...
	   // players nicknames
        ["playersNames", "onEachFrame", {
            {
                if (_x != player && (player distance _x) < viewDistance) then {
                    _color = [1,1,1,1];
                    if (_x call getSide == "cop") then { _color = [0.5,0.5,1,1]; };
        ...
        "Режим администратора включен" call chat;
    };
    case false: {
        ...
        "Режим администратора отключен" call chat;
    };
};
            ...
            if (_tracked_players_count != count(am_admin_mapplayersMarkersList)) then {
                ...
                am_admin_mapplayersMarkersList = [];
                {
                    if (!(player isEqualTo _x)) then {
                        ...
                        _marker_color = [_x] call RRPClient_util_sides_getMarkerColor;
