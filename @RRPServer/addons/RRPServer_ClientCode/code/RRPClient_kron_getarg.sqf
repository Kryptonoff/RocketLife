...
_fl=[_flg] call RRPClient_kron_StrLen;
_out="";
if ((count _this)>2) then {_out=_this select 2};
_arr=[_in] call RRPClient_kron_ArrayToUpper;
if ((count _arr)>0) then {
	for "_i" from 0 to (count _in)-1 do {
		...
		if (_as isEqualType "") then {
			...
			_p = _aa find ":";
			if (_p==_fl) then {
				if (([_as,_fl] call RRPClient_kron_StrLeft)==_flg) then {
					_org = _in select _i;
					_out=[_org,_p+1] call RRPClient_kron_StrMid;
				};
			};
		};
	};
