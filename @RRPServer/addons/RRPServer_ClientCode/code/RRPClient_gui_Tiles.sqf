/*
	...
	Filename: 	RRPClient_gui_Tiles.sqf
...
with uinamespace do {
	...
	if !(isnull _ctrlTileGroup) then {
		...
		//--- Create PP effect
		_ppChromAberration = GVAR_MNS "RRPClient_gui_Tiles_ppChromAberration";
		if (isnil "_ppChromAberration") then {
			_ppChromAberration = ppeffectcreate ["chromAberration",212];
			SVAR_MNS ["RRPClient_gui_Tiles_ppChromAberration",_ppChromAberration];
		};

		_ppDynamicBlur = GVAR_MNS "RRPClient_gui_Tiles_ppDynamicBlur";
		if (isnil "_ppDynamicBlur") then {
			_ppDynamicBlur = ppeffectcreate ["DynamicBlur",213];
			SVAR_MNS ["RRPClient_gui_Tiles_ppDynamicBlur",_ppDynamicBlur];
		};
		...
		_display displayaddeventhandler ["unload","[_this select 0] call RRPClient_gui_Tiles; false"];
