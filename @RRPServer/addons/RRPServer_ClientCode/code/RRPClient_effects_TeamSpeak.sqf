/*
	...
	Filename: 	RRPClient_effects_TeamSpeak.sqf
...
while {true} do {
	...
	if (!(EQUAL(_error,""))) then {
		ADD(life_tfWarn,1);
		_sleep = 5;
		[format[" <t size='2.2' color='#ff0000' align='center'>Внимание!</t><br /><br />%1<br /><br />Это предупреждение номер %2 из 3.<br /><br />Следующая проверка через %3 минут! Исправьте ошибку или будете исключены из игры!</t>",_error,life_tfWarn,_sleep],"error"] spawn RRPClient_system_hint;
	...
	if (life_tfWarn > 2) exitWith {
		[0,format["%1 был исключен с сервера из-за нарушения правила нахождения в TeamSpeak",player call getNickName]] remoteExecCall ["RRPClient_system_broadcast",RCLIENT];
		uiSleep 2;
