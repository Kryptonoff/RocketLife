/*
	...
	Filename: 	RRPClient_effects_CritHit.sqf
...
while {life_critHit} do {
	...
	if (life_critHit && LSALIVE(player) && !(GVAR_RESTRAINED(player)) && NOTINVEH(player)) then {
		...
		["ActsPknlMstpSnonWnonDnon_TreatingInjured_NikitinDead"] call RRPClient_system_animDo;
		...
		switch (true) do {
			...
			default {["amovpercmstpsnonwnondnon"] call RRPClient_system_animDo};
		};
