/*
	...
	Filename: 	RRPClient_system_medicLights.sqf
...
switch (typeOf _vehicle) do {
	...
	case "RRPVehicle_humming_EMS": 				{_lightleft lightAttachObject [_vehicle, [-0.37, 0.0, 0.56]];};
	case "RRPVehicle_ORCA_EMS":		{_lightleft lightAttachObject [_vehicle, [-0.45, 0.7, 0.9]];};
	case "RRPVehicle_HURON_EMS":	{_lightleft lightAttachObject [_vehicle, [0, -1, 0.9]];};
	case "I_Heli_light_03_unarmed_F":		{_lightleft lightAttachObject [_vehicle, [-0.37, 0.0, -0.95]];};
	default {_error = true};
};
...
while {(alive _vehicle)} do {
	if !(_vehicle getVariable ["EMS_beacons",true]) exitWith {};
	if (_leftRed) then {
		_leftRed = false;
