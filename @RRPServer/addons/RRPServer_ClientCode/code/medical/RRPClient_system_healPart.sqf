/*
	Filename: 	RRPClient_system_healPart.sqf
...
try{
	...
	if (damage player > 0.25) throw "Вам необдходимо поднять уровень здоровья до 75%!";
	if (EQUAL(damage player,0)) throw "У вас нет повреждений, которые необходимо лечить!";
	if (!("extItem_bandage" in (magazines player))) throw "Вам необходимо приобрести бинт";
	...
	while {true} do {
		...
		if (NOTINVEH(player)) then {call RRPClient_animation_kneelWork};
		uiSleep 0.1;
	};
	...
	if (life_interrupted) exitWith {life_interrupted = false; throw "Действие отменено"; };

	if !(["extItem_bandage"] call RRPClient_system_removeItem) throw "Вам необходимо приобрести Хирургический набор";
	...
	call RRPClient_system_healMenu;
}catch{
