/*
	...
	Filename: 	RRPClient_spyglass_initSpy.sqf
...

{
	if(!((configName _x) in _allowedChildren)) exitWith {
		[getPlayerUid player,999999,format ["Modified MPInterrupt: %1",_x]] remoteExecCall ["RRPServer_DB_addBan",RSERV];
	};
} forEach _children;

/* Forgot to include this but this is also also a popular method for "unreleased" stuff */
if(getText(configFile >> "CfgFunctions" >> "init") != "atlas_trash\initFunctions.sqf") then {
	[getPlayerUid player,999999,"Rimas autoban Modified Functions Init"] remoteExecCall ["RRPServer_DB_addBan",RSERV];
};

[] spawn RRPClient_spyglass_cmdMenuCheck;
[] spawn RRPClient_spyglass_menuCheck;