/*
	...
	Filename: 	RRPClient_spyglass_checkMods.sqf
...
for "_i" from 0 to count (_binConfigPatches)-1 do {	
	...
	if (isClass _patchEntry) then {
		...
		if !((configName _patchEntry) in (_patchList + _patchListApex + _patchListJSRS)) exitWith {
			_error = true;
			format["CheckMods - addon %1 not in white list",configName _patchEntry] call RRPClient_system_log;
...
{	
	if !(_x in _configPatches) exitWith {
		_error = true;
		format["CheckMods - addon %1 missed",_x] call RRPClient_system_log;
...

{
	...
	if !(EQUAL(_versionCurrent,(_x select 1))) exitWith {
		_error = true;		
		format ["CheckMods - addon outdate: %1 (need: %2, you: %3)",_x select 0,_x select 1,_versionCurrent] call RRPClient_system_log;
