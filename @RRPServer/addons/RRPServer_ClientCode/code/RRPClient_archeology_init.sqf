/*
	...
	Filename: 	RRPClient_archeology_init.sqf
...
"Archeology - Init..." call RRPClient_system_log; //Тут мы запускаем на клиенте сам процесс поиска и добычи археологических предметов
if !(EQUAL(life_arc_currentZones,[])) exitWith {
	titleText ["Загружены сохраненные зоны раскопок","PLAIN"];
	call RRPClient_archeology_setMarkers;
};

"Archeology - Init - All vars cleared" call RRPClient_system_log;
...
format["Archeology - Init - All zones: %1",_allZones] call RRPClient_system_log;

uiSleep 3;

private _zonesMax = ARC_SETTINGS(getNumber,"zonesCount");
format["Archeology - Init - Zones max: %1",_zonesMax] call RRPClient_system_log;

private _currentZones = [];
while {(count _currentZones) < _zonesMax} do {
	_currentZones pushBackUnique (selectRandom _allZones);
};
format["Archeology - Init - Current Zones: %1",_currentZones] call RRPClient_system_log;
...
"Archeology - Init Completed" call RRPClient_system_log;