...
private ["_sessionID","_parameters","_playerSel","_player","_clanID","_clanHash","_titles","_members","_index","_curLevel","_new"];
...
try
{
	...
	_titles = _clanHash getOrDefault ["ranks",[]];
	...
	_new = _curLevel + 1;
	if ((count _titles) < _new) throw "Выбранный член организации уже достиг максимального уровня!";
	...
	if !(isNull _playerSel) then {[_playerSel,"updatePlayerRank",[_clanHash,_new,"повышены"]] call RRPServer_system_sendTo};
	["SuccessTitleAndText", ["Организация","Выбраный член организации был повышен!"]] remoteExecCall ["SmartClient_gui_toaster_addTemplateToast",_player];
	[_clanID,"rank",getPlayerUID _player,format ["%1 повысил %2 до уровня %3",_player call getFullName,((_members select _index) select 1),_new]] call RRPServer_groups_log;
