...
{ 
  ...
  private _players = RRPClientGroupInfo getOrDefault ["members",[]];
  ...
  if !(_post in [1,2]) then
  {
    ...
    if ("canUp" call RRPClient_groups_util_permCheck) then 
    { 
      if !((RRPClientGroupLevel - 1) > _postsellvl) then 
      { 
        throw "Вы не можете повысить в звании выше чем Вы сами!"; 
      };
    }else{
  ...
  ["addRank",[_uid]] call RRPClient_system_send; 
}catch{ 
