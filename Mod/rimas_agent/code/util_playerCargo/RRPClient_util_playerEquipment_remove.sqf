...
try 
{
	...
	_assignedItems = (assignedItems _player) call RRPClient_util_array_toLower;
	if (_lowerCaseItemClassName in _assignedItems) then
	{
		_player unlinkItem _itemClassName; 
		throw true;
	};
 	_primaryWeaponItems = (primaryWeaponItems _player) call RRPClient_util_array_toLower;
	if (_lowerCaseItemClassName in _primaryWeaponItems) then
	{
		_player removePrimaryWeaponItem _itemClassName; 
		throw true;
	};
 	_primaryWeaponMagazines = (primaryWeaponMagazine _player) call RRPClient_util_array_toLower;
	if (_lowerCaseItemClassName in _primaryWeaponMagazines) then
	{
		_player removePrimaryWeaponItem _itemClassName; 
		throw true;
	};
 	_secondaryWeaponItems = (secondaryWeaponItems _player) call RRPClient_util_array_toLower;
	if (_lowerCaseItemClassName in _secondaryWeaponItems) then
	{
		_player removeSecondaryWeaponItem _itemClassName; 
		throw true;
	};
 	_secondaryWeaponMagazines = (secondaryWeaponMagazine _player) call RRPClient_util_array_toLower;
	if (_lowerCaseItemClassName in _secondaryWeaponMagazines) then
	{
		_player removeSecondaryWeaponItem _itemClassName; 
		throw true;
	};
 	_handgunItems = (handgunItems _player) call RRPClient_util_array_toLower;
	if (_lowerCaseItemClassName in _handgunItems) then
	{
		_player removeHandgunItem _itemClassName; 
		throw true;
	};
  	_handgunMagazines = (handgunMagazine _player) call RRPClient_util_array_toLower;
