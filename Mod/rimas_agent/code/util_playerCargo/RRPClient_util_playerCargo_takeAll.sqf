...
if (_target isKindOf "Man") then
{
	if ((uniform _player) isEqualTo "") then
	{
		...
		{
			_targetUniformContent = (uniformContainer _target) call RRPClient_util_containerCargo_serialize;
			_player forceAddUniform (uniform _target);
			[(uniformContainer _player), _targetUniformContent] call RRPClient_util_containerCargo_deserialize;
	...
	if ((vest _player) isEqualTo "") then
	{
		...
		{
			_targetVestContent = (vestContainer _target) call RRPClient_util_containerCargo_serialize;
			_player addVest (vest _target);
			[(vestContainer _player), _targetVestContent] call RRPClient_util_containerCargo_deserialize;
			removeVest _target; 
		};
	};
	if ((backpack _player) isEqualTo "") then
	{
		...
		{
			_targetBackpackContent = (backpackContainer _target) call RRPClient_util_containerCargo_serialize;
			_player addBackpackGlobal (backpack _target);
			[(backpackContainer _player), _targetBackpackContent] call RRPClient_util_containerCargo_deserialize;
			removeBackpackGlobal _target; 
		};
	};
	if !((headgear _target) isEqualTo "") then
	{
		if ([_player, (headgear _target)] call RRPClient_util_playerCargo_add) then
		{
	...
	{
		if ([_player, (goggles _target)] call RRPClient_util_playerCargo_add) then
		{
	...
	{
		if ([_player, (binocular _target)] call RRPClient_util_playerCargo_add) then
		{
	...
	};
	{
		...
		{
			if ([_player, _x] call RRPClient_util_playerCargo_add) then
			{
...
{
	_serializedCargo = _target call RRPClient_util_containerCargo_serialize;
	{
		...
		{
			[_player, _subContainerClassName] call RRPClient_util_playerEquipment_add;
			...
			[_newSubContainerHandle, _subContainerCargo] call RRPClient_util_containerCargo_deserialize;
	...
	forEach (_serializedCargo select 3);
	{
		_weaponItems = _x;
		{
			if ((typeName _x) isEqualTo "STRING") then
			{
				...
				{
					if ([_player, _itemClassName] call RRPClient_util_playerCargo_add) then
					{
			...
			else 
			{
				if !(_x isEqualTo []) then 
				{
					...
					{
						if ([_player, _magazineClassName, _magazineBulletCount] call RRPClient_util_playerCargo_add) then
						{
							_weaponItems set [_forEachIndex, ""]; 
						};
					};
	...
	[_target, _serializedCargo] call RRPClient_util_containerCargo_deserialize;
};