/*
	...
	Filename: RRPClient_util_fetchVehInfo.sqf
...
_msnVar = missionNamespace getVariable format ["EVIC_%1",_class];
if !(isNil "_msnVar") exitWith {missionNamespace getVariable format ["EVIC_%1",_class]}; //проверяем кэщ
...
missionNamespace setVariable [format ["EVIC_%1",_class],_return]; //кэшируем
