...
_container call RRPClient_util_containerCargo_clear;
{
...
forEach (_data select 2);
{
	...
	[_container, _subContainerItemClassName] call RRPClient_util_containerCargo_add;
	_subContainerContainer = ((everyContainer _container) select ((count (everyContainer _container)) - 1)) select 1;
	[_subContainerContainer, _subContainerCargo] call RRPClient_util_containerCargo_deserialize;
}
