...
try{
	switch _mode do {
		...
		case "saveRoute": {
			...
			if (_text isEqualTo "") throw (getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgNoRouteNameGiven"));

			if (count _text > 30) throw (getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteNameLimit"));
			...
			if ({!(_x in _allowed)} count toArray _text > 0) throw (getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgIllegalChars"));
			...
			if (_savedLocation isEqualTo -1) throw (getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteLimit"));
			...
			closeDialog 0;
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteSaved"))] call toastSuccess;
		};
	...
	["Навигатор",_exception] call toastError;
};