...
switch _mode do {
	case "onLoad": {
		...
		MAVV(RoadSigns) = [
			["signt_dangerbendsr.p3d", "scurve", "\rimas_gui\data\nav\data\left-reverse-curve-black.paa"],
			["signt_dangerbendsl.p3d", "scurve", "\rimas_gui\data\nav\data\left-reverse-curve-black.paa"],
			["signt_dangerbendl.p3d", "left_curve", "\rimas_gui\data\nav\data\left-curve-sign-black.paa"],
			["signt_dangerbendr.p3d", "right_curve", "\rimas_gui\data\nav\data\right-curve-sign-black.paa"]
		];

		MAVV(ImagePath) = "\rimas_gui\data\nav\data\";
...

				{
					...
					if ((_currentNodeData param [1, objNull]) isEqualTo ((_inverted param [(count _inverted) - 1]) param [1, objNull])) then {
						if (((_inverted param [(count _inverted) - 1]) param [1, objNull]) distance player < 15 || ((count _inverted) <= 1)) then {
							...
							_three ctrlSetText "\rimas_gui\data\nav\data\flag-map-marker.paa";
							_five ctrlSetText "";
						...
						if (_currentNodeData param [2, "none"] == "left") then {
							...
							if (!_hasSpecifiedStraight) then {
								...
								_three ctrlSetText "\rimas_gui\data\nav\data\arrow-straight.paa";
								_five ctrlSetText "\rimas_gui\data\nav\data\arrow-left.paa";
							};
