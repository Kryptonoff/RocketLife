...
switch _mode do {
	...
	case "start": {
		if !(isNil "MAV_gps_v_currentRoute") exitWith {
			["Навигатор",(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgFinishRouteFirst"))] call toastError;
		};
	...
	case "delete": {
		...
		if !(_curSel isEqualTo -1) then {
			...
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteDeleted"))] call toastInfo;
