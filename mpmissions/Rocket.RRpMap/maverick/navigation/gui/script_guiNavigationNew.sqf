...
try{
	switch _mode do {
		...
		case "commonLocations": {
			...
			if (_roads isEqualTo []) throw (getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgNoRoadsFound"));
			...
			if (isNull _road) throw (getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgInvalidRoadSegment"));
			if ((roadsConnectedTo _road) isEqualTo []) throw (getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgInvalidRoadSegment"));
