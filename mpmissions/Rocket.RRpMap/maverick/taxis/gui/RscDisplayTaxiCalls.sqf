...
switch (_mode) do {
	...
	case "refreshButtonClicked": {
		...
		// Header
		_taxiCallsListbox lnbAddRow ["Клиент", "Дистанция", "Уже ждет", "Место вызова", "Пункт назначени"];
	...
	case "btnOnDuty": {
		...
		// Message
		[["msgNowOnDuty"] call mav_taxi_fnc_getLocalization, true, "\rimas_gui\data\taxi\data\information-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
	...
	case "btnOffDuty": {
		...
		// Message
		[["msgNowOffDuty"] call mav_taxi_fnc_getLocalization, true, "\rimas_gui\data\taxi\data\information-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
	};
	...
	case "btnCancel": {
		...
		[_display] spawn {
			...
			if (_ret) then {
				...
				["Такси",(["msgTaxiFareCancelled"] call mav_taxi_fnc_getLocalization)] call toastInfo;
