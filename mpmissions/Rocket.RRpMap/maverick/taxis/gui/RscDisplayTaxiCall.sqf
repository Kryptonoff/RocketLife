...
switch (_mode) do {
	...
	case "mapDblClick": {
		...
		if (isNull _nearestRoad) then {
			["Такси",(["msgTaxiNoRoadNearFound"] call mav_taxi_fnc_getLocalization)] call toastError;
		} else {
	...
	case "callTaxiClicked": {
		...
		if (_basePrice > 0) then {
			...
			if (!_couldPay) then {
				_exit = true;
				["Такси",(["msgBaseNotAffordable"] call mav_taxi_fnc_getLocalization)] call toastError;
				mav_taxi_v_destination = [];
			};
		};
		...
		["Такси",(["msgTaxiCalled"] call mav_taxi_fnc_getLocalization)] call toastInfo;
	};
	...
	case "cancelTaxiClicked": {
		...
		if (mav_taxi_v_hasCalledTaxi) then {
			if (isNull mav_taxi_v_myDriver) then {
				...
				["Такси",(["msgTaxiCancelled"] call mav_taxi_fnc_getLocalization)] call toastInfo;
				["showUI", [_display]] call RscDisplayTaxiCall;
				mav_taxi_v_destination = [];
