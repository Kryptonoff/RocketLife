...
if (_isDriver) then {
	...
	waitUntil {uiSleep 0.1; mav_taxi_v_callerReceivedTotalDistance};
...
if (_automaticBilling) then {
	...
	if (_isDriver) then {
		...
		waitUntil {uiSleep 0.1; (mav_taxi_v_callerPaid != -1)};
		...
		if (mav_taxi_v_callerPaid == _totalPrice) then {
			...
			if (!_notCompleted) then {
				[format [["msgPaidDriver"] call mav_taxi_fnc_getLocalization, (_totalPrice + _basePrice) toFixed 0], true, "\rimas_gui\data\taxi\data\check-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
			};
		} else {
			// Not full price
			...
			if (!_notCompleted) then {
				[format [["msgNotFullyPaidDriver"] call mav_taxi_fnc_getLocalization, (_totalPrice + _basePrice) toFixed 0, mav_taxi_v_callerPaid], true, "\rimas_gui\data\taxi\data\check-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
			};
		...
		if (_couldPay) then {
			...
			if (!_notCompleted) then {
				[format [["msgPaidCustomer"] call mav_taxi_fnc_getLocalization, (_totalPrice + _basePrice) toFixed 0], true, "\rimas_gui\data\taxi\data\check-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
			};
		} else {
			...
			// Message
			if (!_notCompleted) then {
				[format [["msgNotFullyPaidCustomer"] call mav_taxi_fnc_getLocalization, (_totalPrice + _basePrice) toFixed 0, _maxAmountAvailable], true, "\rimas_gui\data\taxi\data\check-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
			};
	...
	if (!_isDriver) then {
		[format [["msgPayCustomer"] call mav_taxi_fnc_getLocalization, _totalPrice toFixed 2], true, "\rimas_gui\data\taxi\data\check-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
	} else {
		[format [["msgPayDriver"] call mav_taxi_fnc_getLocalization, _totalPrice toFixed 2], true, "\rimas_gui\data\taxi\data\check-symbol.paa"] spawn mav_taxi_fnc_showLargeNotification;
	};
};
