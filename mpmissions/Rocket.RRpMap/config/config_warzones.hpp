...
class CfgWarZone
{
	condition = "!dialog && call RRPClient_groups_isPrem && {(('cop' call RRPClient_groups_getSideLevel) > 1 OR ('civ' call RRPClient_groups_getSideLevel) > 2 OR call life_adminlevel > 3)}";
	cooldownChat = 5; //в минутах
	maxZones = 3;

	costImpulse = 25000;
	impulseTime = 10; //в секундах
	cooldownImpulse = 5; //в минутах
};
